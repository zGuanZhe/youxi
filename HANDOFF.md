...
## ★ v3.17c（2026-08-20 深夜，输入框周围阴影根治：底部区三路暗源清零）

用户复检 v3.17b 后仍报"对话页输入框周围有阴影，首页没有"。排查思路（两页差异反推）：输入框本体两页同组件（自身投影排除）、main surface 本体已透明（实测 rgba(0,0,0,0)）、输入框周围元素栈全透明 → 暗感全部来自 **thread 专属的三个叠加层**，而首页一个都没有：

1. 轻纱罩纵向渐变底沉段（55%~100%，v3.17b 只降到 .12 没清零）
2. 轻纱罩横向暗边（90deg，左右 .14/.10，**全高渲染**——输入框左右侧也被覆盖）
3. scroll-footer 渐变层（v3.17/v3.17b 两轮降 alpha + mask 均未清干净）

**根治**：输入框所在的底部区域（main 高度 82% 以下，输入框实测位于 84%~98%）让三层全部消失——纵向渐变 82% 处 alpha 归 0；横向暗边 `background-size: 100% 82%`（只画到 82% 高度）；scroll-footer 层 `background: transparent + background-image/mask: none` 整层废掉。thread 页上部轻纱氛围完整保留，输入框周围与首页观感一致。

**教训（连续第三轮同主题）**："降 alpha"式的渐进修补永远赶不上用户眼睛——多源叠加的视觉残留只有"归零"一条路。两页/两态对比（"首页没有"）永远是定位的第一线索。

## ★ v3.17b（2026-08-20 深夜，用户复检三连修：阴影块真凶 + 首页标题栏 + 代码块深度）

v3.17 首轮修复后用户复检仍报三处，本轮全部实锤修正：

1. **底部阴影块真凶 = 轻纱罩，不是 scroll-footer**：`main[data-ag-bare]::before` 纵向渐变底部 alpha **.38**、978px 全宽、从 55% 高度就开始变暗——整个下半屏的暗纱，输入框后面+两侧全中。首页 main 无 data-ag-bare（无轻纱罩）→ "首页没有阴影块"与之完全吻合，这才是定位关键。v3.17 首轮只调了 scroll-footer 叠加层（w=845，次要贡献），所以用户看"根本没改"。修复：轻纱罩底沉 .38→**.12**、起点 55%→85%（亮色版 .34→.08 同步）；scroll-footer 层再降 .26→.16 防输入框正后方双重变暗。
2. **首页也有标题栏色块**：对话名栏组件（`h-toolbar` + `draggable` class 组合）在首页（新对话）同样存在（空标题）。v3.17 的 `main[data-ag-bare] > header` 只管 thread 页。修复：改特征选择器 `header[class*="h-toolbar"][class*="draggable"]` 全局隐藏（实测全 DOM 仅对话名栏持有此组合；顶栏 menu bar 是 app-header-tint 族、代码块 sticky header 无 h-toolbar，均不误伤）。
3. **代码块太深**：v3.17 卡片 .88/.82 + tab .45 叠加过深。修复：卡片渐变降到 **.66/.56**、tab 条 .30。

**经验**：用户说"开始界面没有"是黄金线索——用页面间差异反推元素归属，比盯着疑似元素改参数快得多。
